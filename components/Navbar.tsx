    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-6 md:px-12 pointer-events-none">
      <div className="max-w-[1400px] mx-auto flex justify-between items-center pointer-events-auto">
        ...
        <div className="hidden sm:flex items-center gap-1 p-1 rounded-full glass-panel">
          ...
          <a href="#services" onClick={(e) => handleScroll(e, 'services')} className="px-5 py-2.5 text-sm font-normal text-neutral-400 hover:text-white transition-colors rounded-full hover:bg-white/5">Consulting</a>
          ...
          <a href="#" onClick={(e) => { e.preventDefault(); window.dispatchEvent(new CustomEvent('open-contact-modal', { detail: { type: 'consultation' } })); }} className="px-5 py-2.5 text-sm font-normal text-neutral-400 hover:text-white transition-colors rounded-full hover:bg-white/5">Contact</a>
